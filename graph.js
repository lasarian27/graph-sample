...
// Get the first 10 messages in the user's inbox
    const client = getAuthenticatedClient(msalClient, userId);
    return client
        .api("/me/messages?$filter=isRead eq true")
        .select('bodyPreview,receivedDateTime,from,toRecipients,subject')
        ...
        .get((error, messages) => {
          ...
          if (error) {
            ...
            messages.value.forEach((message) => {
              ...
              console.log(`To: ${JSON.stringify(message.toRecipients)}`);
              console.log(`Code: ${message.bodyPreview.replace(/\D/g, "")}`);
            });
          }
        });
  },
